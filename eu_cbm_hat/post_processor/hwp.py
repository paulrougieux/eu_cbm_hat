...
class HWP:
    ...
    @cached_property
    def allocation_to_semifinished(self)-> pandas.DataFrame:
        df1 = hwp_common_input.production_from_domestic_harvest
        df2 = self.fluxes_by_grade
        # merge df1, df2 on year
        # calculate the fractions for 3LRY available in df1
        df['sw_fraction'] = df['sw_dom_tc'] /df['sawlogs']
        df['pp_fraction'] = df['pp_dom_tc'] /df['pulpwood']
        df['wp_fraction'] = df['wp_dom_tc'] /((df['sawlogs'] - df['sw_dom_tc']) + (df['pulpwood']-df['pp_dom_tc'] ))
        # apply the average fractions to var: df['sawlogs'] and df['pulpwood'] in fluxes_by_grade, for from LRY in df1 to the end of the simulation
        # concatenate in an unique dataframe 1900-LSY the dataframe containg df['sw_dom_tc' ],df['pp_dom_tc'],df['wp_dom_tc'] and the df corresponding 
        # variables for the future generated by allocation_to_semifinished

        # to use these for names for variables: 'sw_domestic_tc', 'wp_domestic_tc', 'pp_domestic_tc'
        return df
