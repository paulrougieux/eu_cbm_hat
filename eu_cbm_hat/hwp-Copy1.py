...
class HWP:
    ...
    def fluxes_by_grade(self) -> pandas.DataFrame:
        ...
        # restructuring the supply in LRY 2021-2023 as of 2018-2020
        total_2018_2020 = df[df['year'].between(2018, 2020)].sum(numeric_only=True)
        total_2021_2023 = df[df['year'].between(2021, 2023)].sum(numeric_only=True)
        
        for col in ['pulpwood_broad', 'pulpwood_con', 'sawlogs_broad', 'sawlogs_con']:
            # Calcola le proporzioni per ciascuna colonna
            prop_2018 = df.loc[df['year'] == 2018, col].values[0] / total_2018_2020[col]
            prop_2019 = df.loc[df['year'] == 2019, col].values[0] / total_2018_2020[col]
            prop_2020 = df.loc[df['year'] == 2020, col].values[0] / total_2018_2020[col]
            
            # Calcola i nuovi valori per gli anni 2021, 2022 e 2023
            new_value_2021 = total_2021_2023[col] * prop_2018
            new_value_2022 = total_2021_2023[col] * prop_2019
            new_value_2023 = total_2021_2023[col] * prop_2020
            
            # Assegna i nuovi valori agli anni 2021, 2022 e 2023
            df.loc[df['year'] == 2021, col] = new_value_2021
            df.loc[df['year'] == 2022, col] = new_value_2022
            df.loc[df['year'] == 2023, col] = new_value_2023

        # Separate data before and after the simulation base year
        selector = df["year"] < self.runner.country.base_year
        df_before = df.loc[selector].copy()
        df_after = df.loc[~selector].copy()
        # Calculate the ratio for each year to the previous year for each column
        df_after['pulpwood_broad_ratio'] = df_after['pulpwood_broad'].div(df_after['pulpwood_broad'].shift(1)).replace([np.inf, -np.inf], np.nan).fillna(0)
        df_after['pulpwood_con_ratio'] = df_after['pulpwood_con'].div(df_after['pulpwood_con'].shift(1)).replace([np.inf, -np.inf], np.nan).fillna(0)
        df_after['sawlogs_broad_ratio'] = df_after['sawlogs_broad'].div(df_after['sawlogs_broad'].shift(1)).replace([np.inf, -np.inf], np.nan).fillna(0)
        df_after['sawlogs_con_ratio'] = df_after['sawlogs_con'].div(df_after['sawlogs_con'].shift(1)).replace([np.inf, -np.inf], np.nan).fillna(0)

        columns_to_modify = ['pulpwood_broad','pulpwood_con','sawlogs_broad','sawlogs_con']
        columns_with_fractions = ['pulpwood_broad_ratio','pulpwood_con_ratio','sawlogs_broad_ratio','sawlogs_con_ratio']
        # Calculate a unique ratio between the past n year and the simulation base year
        ...
        # Iterate over the rows and apply the multiplication logic
        for index, row in df_after.iterrows():
            if row['year'] == self.runner.country.base_year:
                for col in columns_to_modify:
                    df_after.at[index, col] = row[col] * df_ratio[col]
            elif row['year'] > self.runner.country.base_year:
                for col, frac_col in zip(columns_to_modify, columns_with_fractions):
                    # Multiply the recalculated value of the previous year by the corresponding ratio
                    df_after.at[index, col] = df_after.at[index - 1, col] * row[frac_col]
    ...
    def fraction_semifinished_n_years_mean(self) -> pandas.DataFrame:
        ...
        cols = [
            ...
            "wp_dom_tc",
            "pp_dom_tc",
            "recycled_paper_prod_tc",
            "recycled_wood_prod_tc",
        ]
        ...
        df["sawlogs"] = df["sawlogs_con"]+ df["sawlogs_broad"]
        df["sw_fraction"] = (
            (df["sw_con_dom_tc"]+ df["sw_broad_dom_tc"])/ (df["sawlogs"])
            .replace([np.inf, -np.inf], 0)
            .fillna(0)
        )
        ...
        # Roundwood can never be converted totally to sawnwood. Fractions always have to
        # be below this value.
        ...
        # total of sw con and broad should reach this threshold, and be under 1 in any case
        sw_selector = df["sw_fraction"] > 0.65
        if any(sw_selector):
            msg = "Check total sawnwood production from sawlogs production for the following years:\n"
            msg += "\n".join(
                f"Year {int(row['year'])} in {row['area']}: "
                f"sw_dom_tc = {row['sw_dom_tc']:.2f}, "
                f"sawlogs = {row['sawlogs']:.2f}, "
                f"Fraction = {row['sw_fraction']:.2f}"
                for _, row in df[sw_selector].iterrows()
            )
            msg += "\nThis temporary warning related to the sw_con_fraction should be an error instead."
            warnings.warn(msg)
    ...
    def fraction_semifinished_scenario(self) -> pandas.DataFrame:
        ...
        df["recycled_paper_prod_tc"] = mean_frac["recycled_paper_prod_tc"]
        df["recycled_wood_prod_tc"] = mean_frac["recycled_wood_prod_tc"]
...
# #########
    ...
    def stock_sink_results(self) -> pandas.DataFrame:
        ...
        ##################################
        output_path = eu_cbm_data_pathlib / "quick_results" / "df_1900.csv"
        df_1900.to_csv(output_path, mode="w", header=True)
