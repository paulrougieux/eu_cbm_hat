...
#        stands[cols] = stands[cols].multiply(1000, axis="index")
        ...
        if irw_salv_avail < remain_irw_vol:
            ...
            msg += f" \nIRW demand corresponds to {prct:.0f}% of the IRW available. FW Demand (remaining after FW salvage)"
            self.parent.log.info(msg)
        ...
        msg = f"Remaining FW demand after salvage, to be satisfied by regular silvuicultural practices, is {remain_fw_vol:.0f} m3."
        msg += f"Collateral FW (generated by IRW dist) {df_irw['fw_colat'].sum():.0f} m3 "
        msg += f"i.e. is {colat_prct:.0f}% of remaining FW demand."
        self.parent.log.info(msg)
